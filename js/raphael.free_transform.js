...
(function(root, factory) {
    ...
    Raphael.fn.freeTransform = function(subject, options, callback) {
        ...
        ft.updateHandles = function() {
            ...
            if (ft.circle) {
                ft.circle.attr({
                    ...
                    r: Math.max(radius.x, radius.y) * ft.opts.distance + 40
                });
            }
         ...
         * Add handles
         */
        ft.showHandles = function(options, callback) {
            ft.hideHandles();
            ft.axes.map(function(axis) {
                ft.handles[axis] = {};
                ...
                ft.handles[axis].disc = paper.circle(ft.attrs.center.x, ft.attrs.center.y, 7).attr({'fill': 'url(img/rotate.png)'})
            });
         ...
         */
        ft.hideHandles = function(opts, callback) {
            var opts = opts || {}
